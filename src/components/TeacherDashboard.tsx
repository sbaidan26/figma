...
type ViewType = 'home' | 'liaison' | 'messaging' | 'teacher-space' | 'absences' | 'notes' | 'schedule' | 'curriculum' | 'textbook' | 'media' | 'photos' | 'agenda' | 'attendance';

type AppIconType = 'coffee' | 'liaison' | 'messaging' | 'absences' | 'notes' | 'schedule' | 'textbook' | 'media' | 'photos';
...
interface AppSection {
  title: string;
  emoji: 'home' | 'book' | 'calendar' | 'art' | 'star' | 'school' | 'teacher';
  bgColor: string;
  cardColor: string;
  apps: AppItem[];
}
...
export function TeacherDashboard({ onLogout }: TeacherDashboardProps) {
  ...
  const sections: AppSection[] = [
    ...
    },
    {
      ...
      apps: [
        ...
        { iconType: 'absences', title: "Absences / Appel", view: 'agenda' },
        { iconType: 'textbook', title: "Programme de l'année", view: 'curriculum' },
      ],
    },
    {
      title: "Vie de classe",
  ...
  const handleTakeAttendance = (courseId: number) => {
    const courseMap: Record<number, { name: string; time: string }> = {
      1: { name: 'Mathématiques', time: '08:30 - 09:30' },
      2: { name: 'Français', time: '10:00 - 11:00' },
      3: { name: 'Sciences', time: '14:00 - 15:30' },
    };

    const course = courseMap[courseId] || { name: 'Cours', time: '09:00 - 12:00' };
  ...
  const getViewTitle = () => {
    switch (currentView) {
      case 'teacher-space': return 'Espace enseignant';
      ...
      case 'curriculum': return 'Programme de l\'année';
      case 'homework': return 'Devoirs';
      case 'resources': return 'Supports de cours';
      case 'photos': return 'Photos de classe';
      case 'agenda': return 'Agenda & Appel';
      case 'attendance': return 'Appel';
      default: return 'Espace Enseignant';
    }
  };

  const renderView = () => {
    if (currentView === 'attendance' && selectedCourse) {
      return (
        <AttendanceView
    ...
    switch (currentView) {
      ...
      case 'absences':
        return <AbsencesView />;
      case 'agenda':
        return <AgendaView onTakeAttendance={handleTakeAttendance} />;
      case 'resources':
        return <CourseResourcesView isTeacher={true} />;
      case 'homework':
        return <HomeworkView role="teacher" />;
      case 'schedule':
        return <ScheduleView role="teacher" />;
      case 'notes':
        return <GradesView />;
      case 'curriculum':
                    ...
                    {sections.map((section, sectionIdx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Apps Grid */}
                          <div className="grid grid-cols-3 gap-3 max-h-[140px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/30 scrollbar-track-transparent">
